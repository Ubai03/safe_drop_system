...
<script>
function openVerificationModal(recipient_id) {
    $.get('../../database/get_attempt_remaining.php', { recipient_id: recipient_id }, function(response) {
        const data = JSON.parse(response);
        let msg = "";

        if (data.status === "success") {
            if (data.attempt_remaining > 0) {
                msg = `You have ${data.attempt_remaining} attempt(s) remaining.`;
            } else {
                msg = "No attempts left. Please wait for admin approval.";
            }
        } else {
            msg = "Awaiting admin approval for your first verification attempt.";
        }

        // Display message inside modal
        $("#attempt_message").text(msg);

        // ✅ Try Bootstrap modal first, fallback to plain display
